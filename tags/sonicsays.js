...
const sonicSaysJob = async (cfg) => {
   ...
   if (cfg?.assyst?.args.length === 0) {
	...
	try {
    const [sonic, font] = await Promise.all([fetch(`https://cdn.discordapp.com/attachments/824020889234440232/1083513327700951093/Untitled160_20230310001307.png`).then(res => res.arrayBuffer()).then(ImageScript.decode), fetch(cfg?.font ?? `https://cdn.discordapp.com/attachments/824020889234440232/1084046492865662996/font.ttf`).then(res => res.arrayBuffer()).then(a => new Uint8Array(a))]);
                                             
const layout = new ImageScript.TextLayout({ maxHeight: 350, maxWidth: 525, wrapStyle: "word", wrapHardBreaks: false, horizontalAlign: "top" });
